...
- `zsh/wrappers.zsh` — wraps `pg_dump`, `psql`, `aws`, `ssh` to transparently decrypt a GPG-encrypted credentials file (`~/.pgpass.gpg`, `~/.aws/credentials.gpg`, `~/.ssh/id_rsa.gpg`) into a short-lived tmp file, run the real command, then delete the tmp file after a 2s delay. Any change here touches how real credentials are handled — be careful with permissions (`chmod 0600`) and cleanup ordering. It also auto-manages SSH tunnels for `psql service=dwh5`/`service=dwh6`: `_pg_tunnel_ensure` (called from `psql_safe`) reuses an already-running tunnel or opens a new one via `ssh_safe -f -N`, tracking state (pid, last-active time) in `~/.cache/pg-tunnels/`. The service→port/host table lives in `zsh/pg_tunnels.zsh`, shared with `scripts/pg-tunnel-reaper.sh`, which a launchd agent (`launchagents/local.pg-tunnel-reaper.plist`, installed via `make install-pg-tunnel-reaper`) runs every 60s to kill tunnels idle for longer than `PG_TUNNEL_IDLE_TIMEOUT`.
...
make install-pg-tunnel-reaper   # install + load the launchd agent that reaps idle psql SSH tunnels
```
